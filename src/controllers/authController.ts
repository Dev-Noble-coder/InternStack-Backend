...
export class AuthController {
  ...
  private setAuthCookies(
    ...
    response.cookie(config.ACCESS_COOKIE_NAME, access, {
      ...options,
      maxAge: config.ACCESS_COOKIE_MAX_AGE_MS,
    });
    response.cookie(config.REFRESH_COOKIE_NAME, refresh, {
      ...options,
  ...
  refresh = async (
    ...
    try {
      const result = await this.authService.refresh(
        request.cookies?.[config.REFRESH_COOKIE_NAME],
      );
      this.setAuthCookies(response, result.access, result.refresh);
  ...
  me = async (request: AuthRequest, response: Response, next: NextFunction) => {
    try {
      response.json({
        user: await this.authService.me(request.identity!.userId),
      });
    } catch (error) {
      next(error);
    }
