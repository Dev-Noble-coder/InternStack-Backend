...
export const verifyAccessToken = (token: string): Identity =>
  jwt.verify(token, config.ACCESS_TOKEN_SECRET) as unknown as Identity;
...
export class TokenService {
  ...
  async rotate(raw: string) {
    ...
    if (!old) {
      const existing = await Session.findOne({ refreshTokenHash: hash(raw) });
      if (existing) await this.revokeAll(existing.userId);
      return { kind: existing ? ("reuse" as const) : ("missing" as const) };
    }
    const next = await this.createSession(old.userId, old.metadata as object);
    await Session.updateOne(
      { _id: old._id },
      { $set: { replacedBy: next.session._id } },
    );
