...
export class AuthCodeService {
  ...
  async verify(
    ...
    if (hashSecret(code) !== record.codeHash) {
      const failed = await AuthCode.findOneAndUpdate(
        {
          _id: record._id,
          usedAt: { $exists: false },
          attempts: { $lt: config.OTP_MAX_ATTEMPTS },
        },
        { $inc: { attempts: 1 } },
        { new: true },
      );
      if (failed && failed.attempts >= config.OTP_MAX_ATTEMPTS)
        await AuthCode.updateOne(
          { _id: failed._id, usedAt: { $exists: false } },
          { $set: { usedAt: new Date() } },
        );
      throw new AppError(400, "Invalid or expired code", "INVALID_CODE");
    }
